...
write.table(meta %>% select(SampleID, everything()), file = "../data/07_ctf/decontam_clean_rf_pup.txt", 
            sep = "\t", quote = F, append = F, row.names = F)

otu <- as(otu_table(prune_samples(meta$SampleID, phylo_decontam_clean_rf)), "matrix")
otu <- otu[rowSums(otu) > 0,]
otu_biom <- make_biom(data=otu)
write_biom(otu_biom,"../data/07_ctf/decontam_clean_rf_pup_otu.biom")
```
...
ord_birthmod <- read.table("../data/07_ctf/weeks_results/Weeks_BirthMode_ordination/38e93f60-6067-4e35-b2cf-f9bd11979756/data/data.tsv",  sep = "\t", header = F) %>% 
  transmute(SampleID = V1, PC1 = V2, PC2 = V3, PC3 = V4) %>% 
  left_join(meta_all, by = "SampleID")
wmeta <- ord_birthmod %>% select(-c(PC1,PC2,PC3))

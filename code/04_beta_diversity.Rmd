...
## test seed
#seed == 5
#inner_join(aa %>% filter(N==5) %>% dplyr::select(-N), 
#           aa_2 %>% filter(N==3) %>% dplyr::select(-N), 
#           by = c("Sample1", "Sample2", "Metric")) %>% 
#  mutate(eq = ifelse(value.x == value.y, "Y", "N")) %>% pull(eq) %>% table()
#seed == 3
#inner_join(aa %>% filter(N==3) %>% dplyr::select(-N), 
#           aa_2 %>% filter(N==1) %>% dplyr::select(-N), 
#           by = c("Sample1", "Sample2", "Metric")) %>% 
#  mutate(eq = ifelse(value.x == value.y, "Y", "N")) %>% pull(eq) %>% table()
# test data.table vs tible
#system.time(bb <- aa %>% dplyr::select(Sample1, Sample2, Metric, value) %>% 
#      group_by(Sample1, Sample2, Metric) %>% summarise(value = mean(value)) %>% ungroup())
#   user  system elapsed 
#  0.887   0.007   0.914 
#system.time(aadt <- as.data.table(aa %>% dplyr::select(Sample1, Sample2, Metric, value)))
#   user  system elapsed 
#  0.010   0.002   0.012
...
temp_out = "~/Dropbox/rutgers/20220722_IFNH_CSR_new/output/beta/"
...
#save(DM_uu,file = "~/Dropbox/rutgers/20220722_IFNH_CSR_new/data/beta_DM_uu.RData")
#save(DM_wu,file = "~/Dropbox/rutgers/20220722_IFNH_CSR_new/data/beta_DM_wu.RData")
#save(DM_bray,file = "~/Dropbox/rutgers/20220722_IFNH_CSR_new/data/beta_DM_bray.RData")
#save(DM_jaccard,file = "~/Dropbox/rutgers/20220722_IFNH_CSR_new/data/beta_DM_jaccard.RData")
```

## load data
```{r}
load(file = "~/Dropbox/rutgers/20220722_IFNH_CSR_new/data/BW_and_phylo.RData")
load(file = "~/Dropbox/rutgers/20220722_IFNH_CSR_new/data/beta_DM_uu.RData")
load(file = "~/Dropbox/rutgers/20220722_IFNH_CSR_new/data/beta_DM_wu.RData")
load(file = "~/Dropbox/rutgers/20220722_IFNH_CSR_new/data/beta_DM_bray.RData")
load(file = "~/Dropbox/rutgers/20220722_IFNH_CSR_new/data/beta_DM_jaccard.RData")
...
for(dd in c("uu","wu","bray","jaccard")){
  ...
  wrda <- dbrda(wdist ~ BirthMode + Sex, data = wmeta)
  wrda_eig <- eigenvals(wrda)
  ...
  wdat_center_BirthMode <- wrda_score$centroids %>% as.data.frame() %>% 
  ...
  wdat_center_Sex <- wrda_score$centroids %>% as.data.frame() %>% 
    rownames_to_column(var = "rowname") %>%
    filter(str_detect(rowname, "^Sex")) %>% 
    transmute(Sex = str_remove(rowname, "Sex"), 
                dbRDA1.sex = dbRDA1, dbRDA2.sex = dbRDA2) 
#  wdat_center_Age_group <- wrda_score$centroids %>% as.data.frame() %>% 
#    rownames_to_column(var = "rowname") %>%
#    filter(str_detect(rowname, "^Age_group")) %>% 
#    transmute(Age_group = str_remove(rowname, "Age_group"), 
#                dbRDA1.age = dbRDA1, dbRDA2.age = dbRDA2)     
  wdat <- wdat_rda %>% 
    left_join(wdat_center_Sex, by = "Sex") %>%
    left_join(wdat_center_BirthMode, by = "BirthMode")
#    left_join(wdat_center_Age_group, by = "Age_group")

  x_lab <- paste0("dbRDA1 (", varRDA1, "%)")
  y_lab <- paste0("dbRDA2 (", varRDA2, "%)")
  
  g <- ggplot(wdat, aes(x = dbRDA1, y = dbRDA2, color = BirthMode)) + 
    geom_point(size = 2) + 
    #geom_segment(aes(x = dbRDA1, y = dbRDA2, xend = dbRDA1.birth, yend = dbRDA2.birth),size = 0.2, alpha = 0.5) + 
    #geom_point(data = wdat_center_BirthMode, aes(x = dbRDA1.birth, y = dbRDA2.birth)) + 
    stat_ellipse(type = "t") + 
    scale_color_manual(values = BirthMode_color) + 
    labs(x = x_lab, y = y_lab, title = "") + 
    facet_wrap(~ Age_group, nrow = 2) +
    theme_bw() + theme(aspect.ratio = 1,
                           ...
                           legend.title = element_text(size=12,color="black",face='bold'),
                           title = element_text(size=12,color="black",face='bold'))  
  ggsave(filename = paste0(temp_out,"dbRDA_",dd,"_","pup_by_birthmode_Age_group.pdf"), device = cairo_pdf, 
           plot = g, width = 7, height = 5, units = "in", dpi = 300)   
}  
```

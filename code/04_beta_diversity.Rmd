...
for(dd in c("uu","wu","bray","jaccard")){
  ...
  ## BirthMode by sex and TimePoint pairwise
  ...
  res_sum <- lapply(1:nrow(lst_df), function(i){
    ...
    d1 <- d1 %>% mutate(Sex = lst_df[i,"Sex"], TimePoint = lst_df[i,"TimePoint"], Variable = "BirthMode")
    
    comp <- combn(wwmeta$BirthMode %>% unique() %>% as.character(),2)
    res <- lapply(1:ncol(comp), function(i){
      wmeta_ss <- wmeta_s %>% filter(BirthMode %in% comp[,i])
      wdist_ss <- dist_subset(wdist_s, wmeta_ss$SampleID)
      calcOmega2_dm(adonis2(wdist_ss ~ BirthMode, data = wmeta_ss, by = "terms")) %>% 
        as.data.frame() %>% mutate(Comp = str_c(comp[1,i], comp[2,i], sep = "-")) %>% 
        filter(!is.na(F))
    }) %>% do.call(rbind, .) %>% 
      mutate(Sex = lst_df[i,"Sex"], TimePoint = lst_df[i,"TimePoint"], fo = "~ BirthMode")    
  ...
  ## BirthMode + sex by TimePoint
  res_sum <- lapply(c(wmeta$TimePoint %>% unique()), function(tp){
    wmeta_s <- wmeta %>% filter(TimePoint == tp)
    wdist_s <- dist_subset(wdist, wmeta_s$SampleID)
    disper <- betadisper(wdist_s, wmeta_s$BirthMode)   
    d1 <- (TukeyHSD(disper, which = "group"))$group %>% as.data.frame() %>% 
      rownames_to_column(var = "Comp")
    d1 <- d1 %>% mutate(Sex = "Both", TimePoint = tp, Variable = "BirthMode")
    disper <- betadisper(wdist_s, wmeta_s$Sex)
    d2 <- (TukeyHSD(disper, which = "group"))$group %>% as.data.frame() %>% 
      rownames_to_column(var = "Comp")
    d2 <- d2 %>% mutate(Sex = "Both", TimePoint = tp, Variable = "Sex")    
    
    comp <- combn(wwmeta$BirthMode %>% unique() %>% as.character(),2)
    res <- lapply(1:ncol(comp), function(i){
      wmeta_ss <- wmeta_s %>% filter(BirthMode %in% comp[,i])
      wdist_ss <- dist_subset(wdist_s, wmeta_ss$SampleID)
      calcOmega2_dm(adonis2(wdist_ss ~ BirthMode + Sex, data = wmeta_ss, by = "terms")) %>% 
        as.data.frame() %>% mutate(Comp = str_c(comp[1,i], comp[2,i], sep = "-")) %>% 
        filter(!is.na(F))
    }) %>% do.call(rbind, .) %>% 
      mutate(Sex = "Both", TimePoint = tp, fo = "~ BirthMode + Sex") 
    res <- rbind(res,
                 calcOmega2_dm(adonis2(wdist_s ~ BirthMode + Sex, data = wmeta_s, by = "terms")) %>% 
                   as.data.frame() %>% mutate(Comp = "All") %>% filter(!is.na(F)) %>% 
                   mutate(Sex = "Both", TimePoint = tp, fo = "~ BirthMode + Sex"))
    list(dispersion = rbind(d1,d2), permanova = res)
  })
  ...
  ## BirthMode + Sex + TimePoint
  disper <- betadisper(wdist, wmeta$BirthMode)   
  d1 <- (TukeyHSD(disper, which = "group"))$group %>% as.data.frame() %>% 
    rownames_to_column(var = "Comp")
  d1 <- d1 %>% mutate(Sex = "Both", TimePoint = "All", Variable = "BirthMode")
  ...
  comp <- combn(wwmeta$BirthMode %>% unique() %>% as.character(),2)
  res <- lapply(1:ncol(comp), function(i){
    wmeta_ss <- wmeta %>% filter(BirthMode %in% comp[,i])
    wdist_ss <- dist_subset(wdist, wmeta_ss$SampleID)
    calcOmega2_dm(adonis2(wdist_ss ~ BirthMode + Sex + TimePoint, data = wmeta_ss, by = "terms")) %>% 
      as.data.frame() %>% mutate(Comp = str_c(comp[1,i], comp[2,i], sep = "-")) %>% 
      filter(!is.na(F))
  }) %>% do.call(rbind, .) %>% 
    mutate(Sex = "Both", TimePoint = "All", fo = "~ BirthMode + Sex + TimePoint") 
  res_permanova_3 <- calcOmega2_dm(adonis2(wdist ~ BirthMode + Sex + TimePoint, data = wmeta, by = "terms")) %>% 
    as.data.frame() %>% mutate(Comp = "All") %>% filter(!is.na(F)) %>% 
    mutate(Sex = "Both", TimePoint = "All", fo = "~ BirthMode + Sex + TimePoint") %>% 
    rbind(res,.)

...
for(dd in c("uu","wu","bray","jaccard")){
  ...
  res <- lapply(1:ncol(comp), function(i){
    ...
    adonis2(wwdist ~ TimePoint, data = wwmeta, by = "terms") %>% 
  ...
  res <- lapply(1:ncol(comp), function(i){
    ...
    wwdist <- dist_subset(wdist, wwmeta$SampleID)
    adonis2(wwdist ~ BirthMode, data = wwmeta, by = "terms") %>% 
      as.data.frame() %>% mutate(comp = str_c(comp[1,i], comp[2,i], sep = " - ")) %>% 
      filter(!is.na(F))
  })
...
for(dd in c("uu","wu","bray","jaccard")){
  ...
  ggsave(filename = paste0(temp_out,dd,"_","pup_by_timepoint_pcoa.pdf"), device = cairo_pdf, 
           plot = g, width = 7, height = 5, units = "in", dpi = 300)
  #PC1 by timepoint
  g <- ggplot(wdat, aes(x = TimePoint, y = PC1)) + 
  ...
  g <- ggplot(wdat, aes(x = TimePoint, y = value, )) + 
    ...
    stat_compare_means(group.by=BirthMode, hide.ns = T) 

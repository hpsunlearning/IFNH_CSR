...
CI_by_group = function(wdat, x, y){
  wk = wdat %>% split(., .[x])
  out1 = sapply(wk, function(df){smean.cl.boot(df[y], conf.int = .95)}) %>% t() %>% as.data.frame() %>% rownames_to_column(var = "var")
  out1
}
...
###TimePointInWeeks
dat <- dat_alpha_pup %>% mutate(grp = str_c(TimePointInWeeks, Sex, sep = "_"))
dat$grp %>% unique()

lapply(dat$grp %>% unique(), function(x){
  x = "3_M"

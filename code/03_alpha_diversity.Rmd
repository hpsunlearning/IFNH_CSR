...
alpha_boot <- aa %>% filter(!is.na(faith)) %>%
  dplyr::select(SampleID, Faith_PD=faith, Observed_ASVs=observed, Shannon_Index=shannon, Pielou_evenness=evenness)
#save(alpha_boot, file = paste0(data_path, "temp_alpha.RData"))
```
...
### compare difference
```{r}
load(file = paste0(data_path, "temp_alpha.RData"))
...
alpha_test %>% mutate(faith_diff = Faith_PD.x - Faith_PD.y,
  ...
  dplyr::select(SampleID, ends_with("diff")) %>% 
...
#save(alpha_boot, alpha_rftb, file = paste0(data_path, "alpha.RData"))
```

## load data
```{r}
load(file = "~/Dropbox/rutgers/20220722_IFNH_CSR_new/data/BW_and_phylo.RData")
load(file = "~/Dropbox/rutgers/20220722_IFNH_CSR_new/data/alpha.RData")
load(file = "~/Dropbox/rutgers/20220722_IFNH_CSR_new/data/BW_responder.RData")
temp_out = "~/Dropbox/rutgers/20220722_IFNH_CSR_new/output/alpha/"
...
g <-ggplot(wdat %>% dplyr::select(SampleID,grp,BodySite,Faith_PD:Pielou_evenness) %>% 

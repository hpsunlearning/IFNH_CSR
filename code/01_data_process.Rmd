...
TimePoint_color = c("Week3"="#fdbe85", "Week6"="#fd8d3c", "Week9-14"="#e6550d", "Week18"="#a63603")
...
meta_all %>% filter(Generation == "F1") %>% 
  group_by(BirthMode, TimePoint, Sex) %>% 
  summarise(N = n(),
            N_family = unique(FamilyNo) %>% length(),
            Count = paste0(N, "(", N_family, ")")) %>%
  ungroup() %>% 
  dplyr::select(BirthMode, Sex, TimePoint, Count) %>% 
  tidyr::spread(key = TimePoint, value = Count) 
```
